...
type arrRiwayat [MAX]Riwayat

func tambah(data *arrBarang, n *int) {
	if *n >= MAX {
...
func tampil(data arrBarang, n int) {
	if n == 0 {
...
func ubah(data *arrBarang, n int) {
	var kode string
...
func hapus(data *arrBarang, n *int) {
	var kode string
...
func masuk(data *arrBarang, n int, log *arrRiwayat, m *int) {
	var kode string
	var jumlah, tgl, thn int
	var bln string
	...
	fmt.Print("Bulan (Teks): ")
	fmt.Scan(&bln)

	fmt.Print("Tahun (Angka): ")
	fmt.Scan(&thn)
...
func keluar(data *arrBarang, n int, log *arrRiwayat, m *int) {
	var kode string
	var jumlah, tgl, thn int
	var bln string
...
func tampilRiwayat(log arrRiwayat, m int) {
	if m == 0 {
		fmt.Println("Belum ada riwayat transaksi masuk/keluar.")
		return
	}

	fmt.Println("\n=============================================================================")
	fmt.Printf("%-4s   %-12s   %-18s   %-10s   %-20s\n", "No", "Kode Barang", "Transaction Type", "Jumlah", "Waktu Transaksi")
	fmt.Println("=============================================================================")
	for i := 0; i < m; i++ {
		fmt.Printf("%-4d   %-12s   %-18s   %-3d kg       %d %s %d\n", i+1, log[i].KodeBarang, log[i].Tipe, log[i].Jumlah, log[i].Tanggal, log[i].Bulan, log[i].Tahun)
	}
	fmt.Println("=============================================================================")
}

func sequentialSearch(data arrBarang, n int) {
	var key string
...
func sortKode(data *arrBarang, n int) {
	for i := 0; i < n-1; i++ {
		min := i
		for j := i + 1; j < n; j++ {
...
func binarySearch(data *arrBarang, n int) {
	if n == 0 {
		fmt.Println("Data kosong.")
		return
	}
...
func selectionSort(data *arrBarang, n int) {
	for i := 0; i < n-1; i++ {
		min := i
		for j := i + 1; j < n; j++ {
...
func insertionSort(data *arrBarang, n int) {
	var pass int
	var temp Barang
...
func statistik(data arrBarang, n int) {
	if n == 0 {
...
func main() {
	var gudangData arrBarang
	var jumlahBarang int
	var logTransaksi arrRiwayat
	...
	for {
		...
		fmt.Println("12. Lihat Riwayat Transaksi")
		...
		switch pilih {
		case 1:
			tambah(&gudangData, &jumlahBarang)
		case 2:
			ubah(&gudangData, jumlahBarang)
		case 3:
			hapus(&gudangData, &jumlahBarang)
		case 4:
			masuk(&gudangData, jumlahBarang, &logTransaksi, &jumlahRiwayat)
		case 5:
			keluar(&gudangData, jumlahBarang, &logTransaksi, &jumlahRiwayat)
		case 6:
			tampil(gudangData, jumlahBarang)
		case 7:
			sequentialSearch(gudangData, jumlahBarang)
		case 8:
			binarySearch(&gudangData, jumlahBarang)
		case 9:
			selectionSort(&gudangData, jumlahBarang)
			fmt.Println("\n[Berhasil diurutkan dari STOK TERKECIL ke TERBESAR]")
			tampil(gudangData, jumlahBarang)
		case 10:
			insertionSort(&gudangData, jumlahBarang)
			fmt.Println("\n[Berhasil diurutkan dari STOK TERBESAR ke TERKECIL]")
			tampil(gudangData, jumlahBarang)
		case 11:
			statistik(gudangData, jumlahBarang)
		case 12:
			tampilRiwayat(logTransaksi, jumlahRiwayat)
		case 0:
